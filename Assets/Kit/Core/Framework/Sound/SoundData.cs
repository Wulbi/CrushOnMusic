...
public class SoundData
{
    ...
    public Transform SourceTransform
    {
        ...
        set
        {
            if (value == null)
            {
                sourceTransform = SoundManager.Instance.Gameobject.transform;
            }
            else
            {
    ...
    public SoundData(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
    {
        // Set unique audio ID
        ...
        Pitch = 1;
        StereoPan = 0;
        if (sourceTransform != null && sourceTransform != SoundManager.Instance.Gameobject.transform)
        {
            SpatialBlend = 1;
        }
        ReverbZoneMix = 1;
    ...
    public void Play(float volume)
    {
        ...
        if (Pooled)
        {
            ...
            bool restoredFromPool = SoundManager.Instance.RestoreAudioFromPool(Type, AudioID);
            if (!restoredFromPool)
            {
                return;
            }
    ...
    public void Update()
    {
        ...
        switch (Type)
        {
            case AudioType.Music:
                {
                    AudioSource.volume = Volume * SoundManager.Instance.GlobalMusicVolume * SoundManager.Instance.GlobalVolume;
                    break;
                }
            case AudioType.Sfx:
                {
                    AudioSource.volume = Volume * SoundManager.Instance.GlobalSfxVolume * SoundManager.Instance.GlobalVolume;
                    break;
                }
        }
